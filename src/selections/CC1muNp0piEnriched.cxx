...
#include "XSecAnalyzer/Selections/CC1muNp0piEnriched.hh"
#include "XSecAnalyzer/Selections/EventCategoriesXp.hh"

CC1muNp0piEnriched::CC1muNp0piEnriched() : SelectionBase( "CC1muNp0piEnriched" ) {
  calc_type = kOpt1;
  this->define_category_map();
  this->define_constants();
}

void CC1muNp0piEnriched::define_constants() {
  this->define_true_FV( 21.5, 234.85, -95.0, 95.0, 21.5, 966.8 );
  this->define_reco_FV( 21.5, 234.85, -95.0, 95.0, 21.5, 966.8 );
}

void CC1muNp0piEnriched::compute_true_observables( AnalysisEvent* Event ) {
  ...
  if ( true_muon && true_lead_p ) {
    ...
    mc_e_had_vis_ = std::accumulate( mc_p3_p_vec_->begin(), mc_p3_p_vec_->end(), 0.0,
                                     [](double a, const TVector3& b)
                                     { return std::move(a) + real_sqrt( b.Mag()*b.Mag() + PROTON_MASS*PROTON_MASS ) - PROTON_MASS; });
  }
}

void CC1muNp0piEnriched::compute_reco_observables( AnalysisEvent* Event ) {
...
bool CC1muNp0piEnriched::define_signal( AnalysisEvent* Event ) {
...
bool CC1muNp0piEnriched::selection( AnalysisEvent* Event ) {
  ...
  for ( int p = 0; p < Event->num_pf_particles_; ++p ) {
    ...
    if ( p == muon_candidate_idx_ ) {
      ...
      if ( muon_mom >= MUON_P_MIN_MOM_CUT ) {
...
int CC1muNp0piEnriched::categorize_event(AnalysisEvent* Event) {
...
void CC1muNp0piEnriched::define_output_branches() {
...
void CC1muNp0piEnriched::define_category_map() {
